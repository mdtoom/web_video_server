    ...
    , handler_group_(async_web_server_cpp::HttpReply::stock_reply(async_web_server_cpp::HttpReply::not_found))
{
    ...
    {
        server_.reset(
            new async_web_server_cpp::HttpServer(address_,
...
void WebVideoServer::cleanup_inactive_streams()
{
    ...
    if (lock)
    {
        ...
        itr_type                                                        new_end = std::partition(
            image_subscribers_.begin(), image_subscribers_.end(), !boost::bind(&ImageStreamer::isInactive, _1));
        if (__verbose)
        {
        ...
        if (type == std::string("ros_compressed"))
        {
            ...
            if (!did_find_compressed_topic)
            {
                RCLCPP_WARN(nh_->get_logger(),
                            "Could not find compressed image topic for %s, falling back to mjpeg",
                            topic.c_str());
                type = "mjpeg";
            }
        }
